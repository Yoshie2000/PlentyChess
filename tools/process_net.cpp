...
struct RawNetworkData {
    ...
    float l2Weights[2 * L2_SIZE][OUTPUT_BUCKETS][2 * L3_SIZE];
    float l2Biases[OUTPUT_BUCKETS][2 * L3_SIZE];
...
struct NetworkData {
    ...
    alignas(ALIGNMENT) float l2Weights[OUTPUT_BUCKETS][4 * L2_SIZE * L3_SIZE];
    alignas(ALIGNMENT) float l2Biases[OUTPUT_BUCKETS][2 * L3_SIZE];
...
void transposePermuteNetwork() {
    ...
    for (int b = 0; b < OUTPUT_BUCKETS; b++) {
        ...
        for (int l2 = 0; l2 < 2 * L2_SIZE; l2++) {
            for (int l3 = 0; l3 < 2 * L3_SIZE; l3++) {
                out.l2Weights[b][l2 * 2 * L3_SIZE + l3] = reinterpret_cast<float*>(tmp.l2Weights)[l2 * OUTPUT_BUCKETS * 2 * L3_SIZE + b * 2 * L3_SIZE + l3];
            }
        }
