...
void TTEntry::update(Hash _hash, Move _bestMove, Depth _depth, Eval _eval, Eval _value, uint8_t _rule50, bool wasPv, int _flags) {
    ...
    if (_flags == TT_EXACTBOUND || (uint16_t)_hash != hash || _depth + ttReplaceTtpvBonus * wasPv + ttReplaceOffset > depth || ((GENERATION_CYCLE + TT_GENERATION_COUNTER - flags) & GENERATION_MASK)) {
        hash = (uint16_t)_hash;
        depth = _depth;
        value = _value;
        eval = _eval;
