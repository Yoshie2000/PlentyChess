...
__attribute_noinline__ Eval NNUE::evaluate(Board* board) {
    ...
    int pieceCount = __builtin_popcount(board->byColor[COLOR_WHITE] | board->byColor[COLOR_BLACK]);
    ...
    for (int i = 0; i < HIDDEN_WIDTH; i++) {
        Eval crelu = std::clamp((int) accumulator.colors[board->stm][i], 0, NETWORK_QA);
        other += crelu * crelu * networkData.outputWeights[bucket][i];
        
        crelu = std::clamp((int) accumulator.colors[1 - board->stm][i], 0, NETWORK_QA);
        other += crelu * crelu * networkData.outputWeights[bucket][i + HIDDEN_WIDTH];
    }

    int unsquared = vecHaddEpi32(sum) / NETWORK_QA + networkData.outputBiases[bucket];
    int unsquaredOther = other / NETWORK_QA + networkData.outputBiases[bucket];
    assert(unsquared == unsquaredOther);
