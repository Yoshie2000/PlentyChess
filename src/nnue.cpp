...
Eval NNUE::evaluate(Board* board) {
    ...
    VecI16* oppAcc = (VecI16*)accumulator->colors[1 - board->stm];

    VecI16* stmWeights = (VecI16*)&networkData->outputWeights[bucket][0];
    VecI16* oppWeights = (VecI16*)&networkData->outputWeights[bucket][L1_SIZE];

    VecI16 reluClipMin = set1Epi16(0);
    VecI16 reluClipMax = set1Epi16(INPUT_QUANT);

    VecI16 sum = set1Epi32(0);
    VecI16 vec0, vec1;
    ...
    int unsquared = vecHaddEpi32(sum) / INPUT_QUANT + networkData->outputBiases[bucket];
