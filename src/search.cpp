...
Eval Thread::checkSearch(Board* board, SearchStack* stack, Eval alpha, Eval beta) {
    ...
    // TT cutoff
    if (!pvNode && ttDepth >= 1 && ttValue != EVAL_NONE && ((ttFlag == TT_UPPERBOUND && ttValue <= alpha) || (ttFlag == TT_LOWERBOUND && ttValue >= beta) || (ttFlag == TT_EXACTBOUND)))
        return ttValue;
...
Eval Thread::search(Board* board, SearchStack* stack, int depth, Eval alpha, Eval beta, bool cutNode) {
    ...
    if (depth <= 1) return checkSearch<nodeType>(board, stack, alpha, beta);
