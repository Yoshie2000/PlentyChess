...
Eval Worker::search(Board* board, SearchStack* stack, Depth depth, Eval alpha, Eval beta, bool cutNode) {
    ...
    while ((move = movegen.nextMove())) {
        ...
        if (!rootNode
            ...
            if (!pvNode && !capture) {
                int lmpDepth = depth + 100 * board->givesCheck(move);

                int lmpMargin;
                if (improving)
                    lmpMargin = 3863191 + 77 * lmpDepth * lmpDepth + 1500 * lmpDepth + moveHistory;
                else
                    lmpMargin = 2958636 + 16 * lmpDepth * lmpDepth + 4500 * lmpDepth + moveHistory;
