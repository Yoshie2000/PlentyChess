...
Eval Worker::search(Board* board, SearchStack* stack, int16_t depth, Eval alpha, Eval beta, bool cutNode) {
    ...
    // TT cutoff
    if (!pvNode && ttDepth >= depth - ttCutOffset + ttCutFailHighMargin * (ttValue >= beta) && ttValue != EVAL_NONE && ((ttFlag == TT_UPPERBOUND && ttValue <= alpha && !cutNode) || (ttFlag == TT_LOWERBOUND && ttValue >= beta && cutNode) || (ttFlag == TT_EXACTBOUND)))
        return ttValue;
