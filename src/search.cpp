...
Eval Thread::search(Board* board, SearchStack* stack, int depth, Eval alpha, Eval beta, bool cutNode) {
    ...
    if ((stack - 1)->movedPiece != Piece::NONE && !(stack - 1)->capture && !(stack - 1)->inCheck && stack->ply > 1) {
        ...
        if (board->stack->pawnHash != board->stack->previous->pawnHash && bonus > 0)
            history.updatePawnHistory(flip(board->stm), (stack - 1)->movedPiece, moveTarget((stack - 1)->move), board->stack->previous->pawnHash, bonus);
    }
