...
movesLoop:
    ...
    while ((move = movegen.nextMove()) != MOVE_NONE) {
        ...
        if (!rootNode
            ...
            int16_t lmrDepth = std::max(0, depth - REDUCTIONS[!capture][depth / 100][moveCount] / (1 + pvNode) - earlyLmrImproving * !improving + 100 * moveHistory / (capture ? earlyLmrHistoryFactorCapture : earlyLmrHistoryFactorQuiet));
