...
Eval Worker::search(Board* board, SearchStack* stack, Depth depth, Eval alpha, Eval beta, bool cutNode) {
    ...
    while ((move = movegen.nextMove())) {
        ...
        if (moveCount > lmrMcBase + lmrMcPv * rootNode - static_cast<bool>(ttMove) && depth >= lmrMinDepth) {
            ...
            bool doResearch = !(ttValue < alpha && ttDepth - lmrResearchSkipDepthOffset >= newDepth && (ttFlag & TT_UPPERBOUND));
            if (value > alpha && reducedDepth < newDepth) {
                ...
                if (!capture) {
                    int bonus = std::min(lmrPassBonusBase + lmrPassBonusFactor * (value > alpha && doResearch ? depth / 100 : reducedDepth / 100), lmrPassBonusMax);
