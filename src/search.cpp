...
Eval Thread::search(Board* board, SearchStack* stack, int depth, Eval alpha, Eval beta, bool cutNode) {
    ...
    if (!rootNode && depth < rfpDepth && std::abs(eval) < EVAL_TBWIN_IN_MAX_PLY && eval - rfpFactor * std::max(0, depth - opponentWorsening - (improving && !board->opponentHasGoodCapture())) >= beta)
