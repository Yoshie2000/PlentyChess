...
TUNE_INT(aspirationWindowDeltaDivisor, 12451, 7500, 17500);
...
TUNE_FLOAT(lmrReductionNoisyBase, -0.1104579566820025f, -2.0f, -0.1f);
TUNE_FLOAT(lmrReductionNoisyFactor, 3.209749067487751f, 2.0f, 4.0f);
TUNE_FLOAT(lmrReductionImportantNoisyBase, -0.23357957515579394f, -2.0f, -0.1f);
TUNE_FLOAT(lmrReductionImportantNoisyFactor, 3.237319076352364f, 2.0f, 4.0f);
TUNE_FLOAT(lmrReductionQuietBase, 1.1368777411312894f, 0.50f, 1.5f);
TUNE_FLOAT(lmrReductionQuietFactor, 2.9597156829516345f, 2.0f, 4.0f);

TUNE_FLOAT(seeMarginNoisy, -23.011839910571545f, -50.0f, -10.0f);
TUNE_FLOAT(seeMarginQuiet, -75.51616358662191f, -100.0f, -50.0f);
TUNE_FLOAT(lmpMarginWorseningBase, 1.8462323240717253f, -1.0f, 2.5f);
TUNE_FLOAT(lmpMarginWorseningFactor, 0.4823404307240103f, 0.1f, 1.5f);
TUNE_FLOAT(lmpMarginWorseningPower, 1.5392764882247214f, 1.0f, 3.0f);
TUNE_FLOAT(lmpMarginImprovingBase, 2.735807858005713f, 2.0f, 5.0f);
TUNE_FLOAT(lmpMarginImprovingFactor, 0.8833184539377286f, 0.5f, 2.0f);
TUNE_FLOAT(lmpMarginImprovingPower, 1.9954091572162302f, 1.0f, 3.0f);

// Search values
TUNE_INT(qsFutilityOffset, 72, 1, 125);
TUNE_INT(qsSeeMargin, -71, -200, 50);

// Pre-search pruning
TUNE_INT(ttCutOffset, 58, -100, 200);
TUNE_INT(ttCutFailHighMargin, 109, 0, 200);

TUNE_INT(iirMinDepth, 305, 100, 1000);
TUNE_INT(iirLowTtDepthOffset, 398, 0, 800);
TUNE_INT(iirReduction, 93, 0, 200);

TUNE_INT(staticHistoryFactor, -84, -500, -1);
TUNE_INT(staticHistoryMin, -157, -1000, -1);
TUNE_INT(staticHistoryMax, 268, 1, 1000);
TUNE_INT(staticHistoryTempo, 27, 1, 200);

TUNE_INT(rfpDepth, 940, 200, 2000);
TUNE_INT(rfpFactor, 76, 1, 250);

TUNE_INT(razoringDepth, 505, 200, 2000);
TUNE_INT(razoringFactor, 287, 1, 1000);

TUNE_INT(nmpMinDepth, 325, 0, 600);
TUNE_INT(nmpRedBase, 390, 100, 800);
TUNE_INT(nmpDepthDiv, 282, 100, 600);
TUNE_INT(nmpMin, 407, 100, 800);
TUNE_INT(nmpDivisor, 214, 10, 600);
TUNE_INT(nmpEvalDepth, 9, 1, 100);
TUNE_INT(nmpEvalBase, 153, 50, 300);

TUNE_INT(probcutReduction, 388, 0, 600);
TUNE_INT(probCutBetaOffset, 182, 1, 500);
TUNE_INT(probCutDepth, 543, 100, 1000);

TUNE_INT(iir2Reduction, 106, 0, 200);

// In-search pruning
TUNE_INT(earlyLmrImproving, 107, 1, 500);

TUNE_INT(earlyLmrHistoryFactorQuiet, 16459, 10000, 20000);
TUNE_INT(earlyLmrHistoryFactorCapture, 14974, 10000, 20000);

TUNE_INT(fpDepth, 1033, 100, 2000);
TUNE_INT(fpBase, 271, 1, 1000);
TUNE_INT(fpFactor, 69, 1, 500);
TUNE_INT(fpPvNode, 36, 1, 250);
TUNE_INT(fpPvNodeBadCapture, 122, 1, 500);

TUNE_INT(fpCaptDepth, 916, 100, 2000);
TUNE_INT(fpCaptBase, 442, 150, 750);
TUNE_INT(fpCaptFactor, 399, 100, 600);

TUNE_INT(historyPruningDepth, 446, 100, 1000);
TUNE_INT(historyPruningFactorCapture, -2274, -8192, -128);
TUNE_INT(historyPruningFactorQuiet, -6271, -8192, -128);

TUNE_INT(extensionMinDepth, 663, 0, 1200);
TUNE_INT(extensionTtDepthOffset, 445, 0, 600);
TUNE_INT(doubleExtensionDepthIncreaseFactor, 95, 0, 200);
TUNE_INT_DISABLED(doubleExtensionMargin, 6, 1, 30);
TUNE_INT(doubleExtensionDepthIncrease, 1119, 200, 2000);
TUNE_INT_DISABLED(tripleExtensionMargin, 41, 25, 100);

TUNE_INT_DISABLED(lmrMcBase, 2, 1, 10);
TUNE_INT_DISABLED(lmrMcPv, 2, 1, 10);
TUNE_INT(lmrMinDepth, 286, 100, 600);

TUNE_INT(lmrReductionOffsetQuietOrNormalCapture, 165, 0, 500);
TUNE_INT(lmrReductionOffsetImportantCapture, 156, 0, 500);
TUNE_INT(lmrCheckQuietOrNormalCapture, 87, 0, 500);
TUNE_INT(lmrCheckImportantCapture, 70, 0, 500);
TUNE_INT(lmrTtPvQuietOrNormalCapture, 180, 0, 500);
TUNE_INT(lmrTtPvImportantCapture, 184, 0, 500);
TUNE_INT(lmrCutnode, 249, 0, 500);
TUNE_INT(lmrTtpvFaillowQuietOrNormalCapture, 78, 0, 500);
TUNE_INT(lmrTtpvFaillowImportantCapture, 106, 0, 500);
TUNE_INT(lmrCorrectionDivisorQuietOrNormalCapture, 141317, 10000, 200000);
TUNE_INT(lmrCorrectionDivisorImportantCapture, 144826, 10000, 200000);
TUNE_INT(lmrQuietHistoryDivisor, 27154, 10000, 30000);
TUNE_INT(lmrHistoryFactorCapture, 3052368, 2500000, 4000000);
TUNE_INT(lmrHistoryFactorImportantCapture, 3015744, 2500000, 4000000);
TUNE_INT(lmrImportantCaptureOffset, 107, 0, 500);
TUNE_INT(lmrImportantBadCaptureOffset, 106, 0, 500);
TUNE_INT(lmrImportantCaptureFactor, 50, 0, 250);
TUNE_INT(lmrQuietPvNodeOffset, 14, 0, 250);
...
TUNE_INT(postlmrOppWorseningReduction, 138, 0, 200);
...
TUNE_INT(lmrResearchSkipDepthOffset, 426, 0, 800);

TUNE_INT(lmrPassBonusBase, -277, -500, 500);
TUNE_INT(lmrPassBonusFactor, 187, 1, 500);
TUNE_INT(lmrPassBonusMax, 1217, 32, 4096);

TUNE_INT(historyDepthBetaOffset, 226, 1, 500);

TUNE_INT(lowDepthPvDepthReductionMin, 422, 0, 800);
TUNE_INT(lowDepthPvDepthReductionMax, 1040, 0, 2000);
