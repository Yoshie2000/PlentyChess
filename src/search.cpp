...
movesLoop:
    ...
    if (failLow && !rootNode && !excluded && (stack - 1)->movedPiece != Piece::NONE) {
        Move previousMove = (stack - 1)->move;
        bool previousCapture = board->stack->capturedPiece != Piece::NONE;
        int bonus = std::min(75 * depth / 10, 75);

        if (!previousCapture) {
            history.updateQuietHistory(previousMove, flip(board->stm), board, board->stack->previous, bonus);
        }
    }
