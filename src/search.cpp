...
Eval Worker::search(Board* board, SearchStack* stack, Depth depth, Eval alpha, Eval beta, bool cutNode) {
    ...
    while ((move = movegen.nextMove())) {
        ...
        if (moveCount > lmrMcBase + lmrMcPv * rootNode - static_cast<bool>(ttMove) && depth >= lmrMinDepth && newDepth >= 100) {
            Depth reduction = REDUCTIONS[int(capture) + int(importantCapture)][depth / 100][moveCount];
            ...
            Depth reducedDepth = std::min(std::max(newDepth - reduction, 100), newDepth + lmrPotentialExtension) + lmrPvNodeExtension * pvNode;
