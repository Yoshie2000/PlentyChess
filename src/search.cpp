...
movesLoop:
    ...
    if (!board->stack->checkers && (bestMove == MOVE_NONE || !board->isCapture(bestMove) || !board->isCapture(ttMove)) && (!failHigh || bestValue > stack->staticEval) && (!failLow || bestValue <= stack->staticEval)) {
