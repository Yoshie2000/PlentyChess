...
Eval Thread::qsearch(Board* board, SearchStack* stack, Eval alpha, Eval beta) {
    ...
    if (board->stack->checkers) {
        ...
        stack->staticEval = bestValue = ttMove != MOVE_NONE ? unadjustedEval : history.correctStaticEval(unadjustedEval, board, stack);
...
Eval Thread::search(Board* board, SearchStack* stack, int depth, Eval alpha, Eval beta, bool cutNode) {
    ...
    if (board->stack->checkers) {
        ...
        eval = stack->staticEval = ttMove != MOVE_NONE ? unadjustedEval : history.correctStaticEval(unadjustedEval, board, stack);
