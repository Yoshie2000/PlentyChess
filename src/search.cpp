...
// Time management
TUNE_FLOAT(tmInitialAdjustment, 1.1054841533532886f, 0.5f, 1.5f);
TUNE_INT(tmBestMoveStabilityMax, 18, 10, 30);
TUNE_FLOAT(tmBestMoveStabilityBase, 1.492667581448128f, 0.75f, 2.5f);
TUNE_FLOAT(tmBestMoveStabilityFactor, 0.05015966971569908f, 0.001f, 0.1f);
TUNE_FLOAT(tmEvalDiffBase, 0.9639878961263904f, 0.5f, 1.5f);
TUNE_FLOAT(tmEvalDiffFactor, 0.009129307324064322f, 0.001f, 0.1f);
TUNE_INT(tmEvalDiffMin, -7, -250, 50);
TUNE_INT(tmEvalDiffMax, 47, -50, 250);
TUNE_FLOAT(tmNodesBase, 1.6583179553793064f, 0.5f, 5.0f);
TUNE_FLOAT(tmNodesFactor, 0.8762573043400358f, 0.1f, 2.5f);
...
TUNE_FLOAT(seeMarginQuiet, -72.54715174170622f, -100.0f, -50.0f);
TUNE_FLOAT(lmpMarginWorseningBase, 1.806550741329393f, -1.0f, 2.5f);
TUNE_FLOAT(lmpMarginWorseningFactor, 0.37352446272678974f, 0.1f, 1.5f);
TUNE_FLOAT(lmpMarginWorseningPower, 1.755595753320366f, 1.0f, 3.0f);
TUNE_FLOAT(lmpMarginImprovingBase, 2.8521517223823984f, 2.0f, 5.0f);
TUNE_FLOAT(lmpMarginImprovingFactor, 0.9816608282030527f, 0.5f, 2.0f);
TUNE_FLOAT(lmpMarginImprovingPower, 1.9193134002314318f, 1.0f, 3.0f);

// Search values
TUNE_INT(qsFutilityOffset, 63, 1, 125);
TUNE_INT(qsSeeMargin, -102, -200, 50);

// Pre-search pruning
TUNE_INT_DISABLED(iirMinDepth, 4, 1, 10);

TUNE_INT(worseningOffset, 16, 1, 50);
...
TUNE_INT(razoringFactor, 220, 1, 1000);

TUNE_INT_DISABLED(nmpRedBase, 3, 1, 5);
TUNE_INT_DISABLED(nmpDepthDiv, 3, 1, 6);
TUNE_INT_DISABLED(nmpMin, 3, 1, 10);
TUNE_INT(nmpDivisor, 49, 10, 1000);

TUNE_INT(probCutBetaOffset, 191, 1, 500);
TUNE_INT_DISABLED(probCutDepth, 5, 1, 15);

// In-search pruning
TUNE_INT(earlyLmrHistoryFactorQuiet, 16654, 128, 32768);
TUNE_INT(earlyLmrHistoryFactorCapture, 14715, 128, 32768);

TUNE_INT_DISABLED(fpDepth, 11, 1, 20);
TUNE_INT(fpBase, 223, 1, 1000);
TUNE_INT(fpFactor, 99, 1, 500);

TUNE_INT_DISABLED(fpCaptDepth, 9, 1, 20);
TUNE_INT(fpCaptBase, 472, 150, 750);
TUNE_INT(fpCaptFactor, 337, 100, 600);

TUNE_INT_DISABLED(historyPruningDepth, 4, 1, 15);
TUNE_INT(historyPruningFactorCapture, -1616, -8192, -128);
TUNE_INT(historyPruningFactorQuiet, -5465, -8192, -128);
...
TUNE_INT(lmrHistoryFactorQuiet, 17844, 128, 32768);
TUNE_INT(lmrHistoryFactorCapture, 19430, 128, 32768);
TUNE_INT(lmrDeeperBase, 40, 1, 100);
TUNE_INT_DISABLED(lmrDeeperFactor, 2, 0, 10);

TUNE_INT(lmrPassBonusBase, -193, -500, 500);
TUNE_INT(lmrPassBonusFactor, 172, 1, 500);
TUNE_INT(lmrPassBonusMax, 1025, 32, 4096);

TUNE_INT(historyBonusBase, 3, -500, 500);
TUNE_INT(historyBonusFactor, 207, 1, 500);
TUNE_INT(historyBonusMax, 1913, 32, 4096);
TUNE_INT(historyBonusBetaOffset, 267, 1, 500);

TUNE_INT(correctionHistoryFactor, 170, 32, 512);
...
int valueToTT(int value, int ply) {
    ...
    if (value > EVAL_TB_WIN_IN_MAX_PLY) value += ply;
    else if (value < -EVAL_TB_WIN_IN_MAX_PLY) value -= ply;
    return value;
}
...
uint32_t probeTB(Board* board) {
    if (static_cast<unsigned int>(BB::popcount(board->byColor[Color::WHITE] | board->byColor[Color::BLACK])) > TB_LARGEST)
        return TB_RESULT_FAILED;

    return tb_probe_wdl(
        board->byColor[Color::WHITE], board->byColor[Color::BLACK],
        board->byPiece[Piece::KING], board->byPiece[Piece::QUEEN], board->byPiece[Piece::ROOK],
        board->byPiece[Piece::BISHOP], board->byPiece[Piece::KNIGHT], board->byPiece[Piece::PAWN],
...
Move moveFromTbProbeRoot(Board* board, uint32_t tbResult) {

    constexpr Piece PROMO_TYPE_TABLE[5] = {
      Piece::NONE,
      Piece::QUEEN,
      Piece::ROOK,
      Piece::BISHOP,
    ...
    constexpr PromotionType PROMOTION_TYPE[6] = { 0b111111111111, PROMOTION_KNIGHT, PROMOTION_BISHOP, PROMOTION_ROOK, PROMOTION_QUEEN, 0b111111111111 };

    const Square origin = (Square)TB_GET_FROM(tbResult);
    const Square target = (Square)TB_GET_TO(tbResult);
    const Piece promoType = PROMO_TYPE_TABLE[TB_GET_PROMOTES(tbResult)];

    if (promoType != Piece::NONE)
        return createMove(origin, target) | PROMOTION_TYPE[promoType];
...
movesLoopQsearch:
    ...
    while ((move = movegen.nextMove()) != MOVE_NONE) {
        ...
        if (futilityValue > -EVAL_INFINITE) { // Only prune when not in check
            if (bestValue >= -EVAL_TB_WIN_IN_MAX_PLY
                && futilityValue <= alpha
                && !SEE(board, move, 1)
                ) {
...
Eval Thread::search(Board* board, SearchStack* stack, int depth, Eval alpha, Eval beta, bool cutNode) {
    ...
    // Probe tablebases
    uint32_t tbResult = (rootNode || excluded) ? TB_RESULT_FAILED : probeTB(board);
    ...
    if (tbResult != TB_RESULT_FAILED) {
        ...
        if (tbResult == TB_LOSS) {
            tbScore = stack->ply - EVAL_TB_WIN;
            tbBound = TT_UPPERBOUND;
        }
        else if (tbResult == TB_WIN) {
            tbScore = EVAL_TB_WIN - stack->ply;
        ...
        if ((tbBound == TT_EXACTBOUND) || (tbBound == TT_LOWERBOUND ? tbScore >= beta : tbScore <= alpha)) {
            ttEntry->update(board->stack->hash, MOVE_NONE, std::min(depth + 6, MAX_PLY - 2), EVAL_NONE, tbScore, ttPv, tbBound);
            return tbScore;
        }
    ...
    if (!rootNode && depth < rfpDepth && std::abs(eval) < EVAL_TB_WIN_IN_MAX_PLY && eval - rfpFactor * (depth - improving) >= beta)
        return (eval + beta) / 2;
    ...
    if (!pvNode
        ...
        if (nullValue >= beta) {
            if (nullValue > EVAL_TB_WIN_IN_MAX_PLY)
                nullValue = beta;
    ...
    // ProbCut
    probCutBeta = std::min(beta + probCutBetaOffset, EVAL_TB_WIN_IN_MAX_PLY - 1);
    if (!pvNode
        && !excluded
        && depth > probCutDepth
        && std::abs(beta) < EVAL_TB_WIN_IN_MAX_PLY - 1
        && !(ttDepth >= depth - 3 && ttValue != EVAL_NONE && ttValue < probCutBeta)) {

        assert(probCutBeta > beta);
        assert(probCutBeta < EVAL_TB_WIN_IN_MAX_PLY);
...
movesLoop:
    ...
    while ((move = movegen.nextMove()) != MOVE_NONE) {
        ...
        if (!pvNode
            && bestValue > -EVAL_TB_WIN_IN_MAX_PLY
            && board->hasNonPawns()
            ) {
        ...
        if (doExtensions
            ...
            && std::abs(ttValue) < EVAL_TB_WIN_IN_MAX_PLY
            && ttDepth >= depth - 3
            ) {
        ...
        if (value > bestValue) {
            ...
            if (value > alpha) {
                ...
                if (depth > 4 && depth < 10 && beta < EVAL_TB_WIN_IN_MAX_PLY && value > -EVAL_TB_WIN_IN_MAX_PLY)
                    depth--;
            }
        }
    ...
    for (int depth = 1; depth <= maxDepth; depth++) {
        ...
        for (int rootMoveIdx = 0; rootMoveIdx < multiPvCount; rootMoveIdx++) {
            ...
            while (true) {
                ...
                if (value >= EVAL_TB_WIN_IN_MAX_PLY) {
                    beta = EVAL_INFINITE;
                    failHighs = 0;
                }
...
void Thread::printUCI(Thread* thread, int multiPvCount) {
    ...
    for (auto& thread : threadPool->threads) {
        tbHits += thread.get()->searchData.tbHits;
    }
    int64_t nps = ms == 0 ? 0 : nodes / ((double)ms / 1000);

    for (int rootMoveIdx = 0; rootMoveIdx < multiPvCount; rootMoveIdx++) {
        RootMove rootMove = thread->rootMoves[rootMoveIdx];
        std::cout << "info depth " << rootMove.depth << " seldepth " << rootMove.selDepth << " score " << formatEval(rootMove.value) << " multipv " << (rootMoveIdx + 1) << " nodes " << nodes << " tbhits " << tbHits << " time " << ms << " nps " << nps << " hashfull " << TT.hashfull() << " pv ";
    ...
    if (threadPool->threads.size() > 1 && UCI::Options.multiPV.value == 1) {
        ...
        for (auto& th : threadPool->threads) {
            ...
            for (size_t rmi = 0; rmi < thread->rootMoves.size(); rmi++) {
                ...
                if (std::abs(bestValue) >= EVAL_TB_WIN_IN_MAX_PLY) {
                ...
                else if (thValue >= EVAL_TB_WIN_IN_MAX_PLY) {
                    bestThread = thread;
                    bestMoveIdx = rmi;
                }
