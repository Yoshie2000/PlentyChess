...
class History {
    ...
    int16_t correctionHistory[KING_BUCKETS][KING_BUCKETS][CORRECTION_HISTORY_SIZE];
...
private:
    ...
    inline Eval getCorrectionHistory(Board* board) {
        return correctionHistory[getKingBucket(board->stm, lsb(board->byColor[board->stm] & board->byPiece[Piece::KING])).index][getKingBucket(flip(board->stm), lsb(board->byColor[flip(board->stm)] & board->byPiece[Piece::KING])).index][board->stack->pawnHash & (CORRECTION_HISTORY_SIZE - 1)];
    }
