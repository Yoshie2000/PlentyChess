...
class History {
    ...
    int16_t correctionHistory[2][KING_BUCKETS][CORRECTION_HISTORY_SIZE];
...
private:
    ...
    inline Eval getCorrectionHistory(Board* board) {
        return correctionHistory[board->stm][getKingBucket(board->stm, lsb(board->byColor[board->stm] & board->byPiece[Piece::KING])).index][board->stack->pawnHash & (CORRECTION_HISTORY_SIZE - 1)];
    }
