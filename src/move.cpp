...
Move MoveGen::nextMove() {
    ...
    case GEN_STAGE_CAPTURES:
        ...
        while (returnedMoves < generatedMoves) {
            ScoredMove move = pickBest(moveList, returnedMoves++, generatedMoves);
    ...
    case GEN_STAGE_REMAINING:
        ...
        if (returnedMoves < generatedMoves)
            return pickBest(moveList, returnedMoves++, generatedMoves).move;
...
ScoredMove MoveGen::pickBest(ScoredMove* moveList, int beginIndex, int endIndex) {
    int bestIndex = beginIndex;
    int bestScore = moveList[bestIndex].score;

    for (int i = beginIndex + 1; i < endIndex; i++) {
        int score = moveList[i].score;
        if (score > bestScore) {
            bestScore = score;
            bestIndex = i;
        }
    ...
    ScoredMove bestMove = moveList[bestIndex];
    moveList[bestIndex] = moveList[beginIndex];
    return bestMove;
}
