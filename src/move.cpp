...
int MoveGen::scoreGoodCaptures(int beginIndex, int endIndex) {
    for (int i = beginIndex; i < endIndex; i++) {
        ...
        moveListScores[i] = 5 * score + *history->getCaptureHistory(board, move);
    }
    return endIndex;
}
...
void MoveGen::scoreBadCaptures() {
    for (int i = 0; i < generatedBadCaptures; i++) {
        ...
        badCaptureScores[i] = 5 * (PIECE_VALUES[board->pieces[moveTarget(move)]] - PIECE_VALUES[board->pieces[moveOrigin(move)]]) + *history->getCaptureHistory(board, move);;
    }
}
