...
int MoveGen::scoreQuiets(int beginIndex, int endIndex) {
    ...
    for (int i = beginIndex; i < endIndex; i++) {
        ...
        if (searchStack)
            ...
            moveListScores[i] = history->getHistory(board, mctsNode, move, false) + threatScore;
    }
    return endIndex;
}
