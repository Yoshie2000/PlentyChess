...
Eval History::correctStaticEval(Eval eval, Board* board, SearchStack* searchStack) {
    ...
    int64_t contEntry = continuationCorrectionHistory[(searchStack - 1)->pieceTo][(searchStack - 2)->pieceTo];
...
void History::updateCorrectionHistory(Board* board, SearchStack* searchStack, int16_t bonus) {
    ...
    // Continuation
    scaledBonus = bonus - continuationCorrectionHistory[(searchStack - 1)->pieceTo][(searchStack - 2)->pieceTo] * std::abs(bonus) / CORRECTION_HISTORY_LIMIT;
    continuationCorrectionHistory[(searchStack - 1)->pieceTo][(searchStack - 2)->pieceTo] += scaledBonus;
}
