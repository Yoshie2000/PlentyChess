...
int History::getHistory(Board* board, BoardStack* boardStack, SearchStack* searchStack, Move move, bool isCapture) {
    if (isCapture) {
        ...
        return getQuietHistory(move, board->stm, board, boardStack) + 2 * getContinuationHistory(searchStack, board->stm, board->pieces[moveOrigin(move)], move) + getPawnHistory(board->stm, board->stack, board->pieces[moveOrigin(move)], move);
    }
}
...
int16_t History::getPawnHistory(Color stm, BoardStack* stack, Piece movedPiece, Move move) {
    return pawnHistory[stack->pawnHash & (PAWN_HISTORY_SIZE - 1)][stm][movedPiece][moveTarget(move)];
}

void History::updatePawnHistory(Color stm, BoardStack* stack, Piece movedPiece, Move move, int16_t bonus) {
    int16_t scaledBonus = bonus - getPawnHistory(stm, stack, movedPiece, move) * std::abs(bonus) / 32000;
...
void History::updateQuietHistories(Board* board, BoardStack* boardStack, SearchStack* stack, Move move, int moveSearchCount, int16_t bonus, int16_t malus, Move* quietMoves, int* quietSearchCount, int quietMoveCount) {
    ...
    updatePawnHistory(board->stm, board->stack, board->pieces[moveOrigin(move)], move, bonus * moveSearchCount);
    ...
    for (int i = 0; i < quietMoveCount; i++) {
        ...
        updatePawnHistory(board->stm, board->stack, board->pieces[moveOrigin(qMove)], qMove, -malus * quietSearchCount[i]);
    }
}
