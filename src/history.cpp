...
void History::updateCorrectionHistory(Board* board, int16_t bonus) {
    ...
    correctionHistory[board->stm][getKingBucket(board->stm, lsb(board->byColor[board->stm] & board->byPiece[Piece::KING])).index][getKingBucket(flip(board->stm), lsb(board->byColor[flip(board->stm)] & board->byPiece[Piece::KING])).index][board->stack->pawnHash & (CORRECTION_HISTORY_SIZE - 1)] += scaledBonus;
}
