...
int History::getHistory(Board* board, BoardStack* boardStack, SearchStack* searchStack, Move move, bool isCapture) {
    if (isCapture) {
        ...
        return getQuietHistory(move, board->stm, board, boardStack) + 2 * getContinuationHistory(searchStack, board->stm, board->pieces[moveOrigin(move)], move) + getPawnHistory(boardStack, board->stm, board->pieces[moveOrigin(move)], move);
    }
}
...
int16_t History::getPawnHistory(BoardStack* stack, Color stm, Piece movedPiece, Move move) {
    return pawnHistory[stack->pawnHash & (PAWN_HISTORY_SIZE - 1)][stm][movedPiece][moveTarget(move)];
}

void History::updatePawnHistory(BoardStack* stack, Color stm, Piece movedPiece, Move move, int16_t bonus) {
    int16_t scaledBonus = bonus - getPawnHistory(stack, stm, movedPiece, move) * std::abs(bonus) / 32000;
    pawnHistory[stack->pawnHash & (PAWN_HISTORY_SIZE - 1)][stm][movedPiece][moveTarget(move)] += scaledBonus;
}
...
void History::updateQuietHistories(int depth, Board* board, BoardStack* boardStack, SearchStack* stack, Move move, int moveSearchCount, Move* quietMoves, int* quietSearchCount, int quietMoveCount) {
    ...
    updatePawnHistory(boardStack, board->stm, board->pieces[moveOrigin(move)], move, pawnHistBonus * moveSearchCount);
    ...
    for (int i = 0; i < quietMoveCount; i++) {
        ...
        updatePawnHistory(boardStack, board->stm, board->pieces[moveOrigin(qMove)], qMove, -pawnHistMalus * quietSearchCount[i]);
    }
}
