...
int History::getHistory(Board* board, MCTSNode* mctsNode, Move move, bool isCapture) {
    if (isCapture) {
        return *getCaptureHistory(board, move);
    }
    else {
        return getQuietHistory(move, board->stm, board) + 2 * getContinuationHistory(mctsNode, board->stm, board->pieces[moveOrigin(move)], move) + getPawnHistory(board, move);
    }
}
